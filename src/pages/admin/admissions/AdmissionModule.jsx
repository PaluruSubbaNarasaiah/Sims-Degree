...
const MOCK_EXISTING_PARENTS = [
  {
    ...
    childrenCount: 2,
    image: 'https://via.placeholder.com/150/FF0000/FFFFFF?text=AS', // Example image URL
  },
  {
    parentId: 'P002',
    name: 'Bob Johnson',
    ...
    childrenCount: 1,
    image: 'https://via.placeholder.com/150/0000FF/FFFFFF?text=BJ', // Example image URL
  },
];
...
const MOCK_EXISTING_STUDENTS = [
  ...
  },
  {
    ...
    previousSchoolAddress: '789 Pine Ln, Old Town',
...
const STUDENT_TYPE_OPTIONS = Object.values(MOCK_EXISTING_STUDENTS.reduce((acc, student) => ({ ...acc, [student.studentType.value]: student.studentType }), {}));
// --- End Mock Data ---
...
function AdmissionModule({ onSave, existingParents = MOCK_EXISTING_PARENTS, existingStudents = MOCK_EXISTING_STUDENTS, uploadFile }) {
  ...
  const validateStudentForm = (student, index) => {
    ...
    if (!student.degreeProgram) errors.degreeProgram = 'Degree Program is required';
    if (!student.studentType) errors.studentType = 'Student Type is required';

    if (student.studentType?.value === 'Transfer Student') {
                      ...
                      {foundExistingChildren.map((child, idx) => (
                        <div key={idx} className="flex items-center gap-4 p-3 bg-white rounded-md shadow-sm border border-purple-100">
                          ...
                          <div className="flex-grow">
                            ...
                            <p className="text-sm text-gray-600">Degree Program: {child.degreeProgram?.label || child.degreeProgram} | Gender: {child.gender?.label || child.gender}</p>
                          </div>
              ...
              <section className="space-y-8">
                ...
                {studentsData.map((student, index) => (
                  <div key={student.id} className="border border-green-200 p-6 rounded-lg bg-green-50 mb-6 relative shadow-md">
                    ...
                    {student.studentType?.value === 'Transfer Student' && (
