...
for (let i = 1; i <= 8; i++) {
  SEMESTER_OPTIONS.push({ label: `Semester ${i}`, value: i });
}
...
const ViewEditStudent = ({ onClose, onSave, data, editable = false, existingStudents, uploadFile }) => {
  ...
  const validateForm = async () => {
    ...
    if (studentType === 'Migrated Student') {
      if (!previousSchoolName.trim()) newErrors.previousSchoolName = 'Previous College Name is required for Migrated Students';
      if (!previousSchoolAddress.trim()) newErrors.previousSchoolAddress = 'Previous College Address is required for Migrated Students';
      if (!previousSchoolStartDate) newErrors.previousSchoolStartDate = 'Previous College Start Date is required for Migrated Students';
      if (!previousSchoolEndDate) newErrors.previousSchoolEndDate = 'Previous College End Date is required for Migrated Students';
    }
      ...
      <DialogContent dividers>
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Program {editable && <span className="text-red-500">*</span>}</label>
              <Select options={PROGRAM_OPTIONS} value={getSelectValue(PROGRAM_OPTIONS, formData.program)} onChange={(selected) => handleSelectChange('program', selected)} placeholder="Select Program"
                className={`mt-1 basic-single ${errors.program ? 'border-red-500' : ''}`} classNamePrefix="select" isDisabled={!editable} />
              {errors.program && <p className="text-red-500 text-xs mt-1">{errors.program}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700">Semester {editable && <span className="text-red-500">*</span>}</label>
              <Select options={SEMESTER_OPTIONS} value={getSelectValue(SEMESTER_OPTIONS, formData.semester)} onChange={(selected) => handleSelectChange('semester', selected)} placeholder="Select Semester"
            ...
            {formData.studentType === 'Migrated Student' && (
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Previous College Name {editable && <span className="text-red-500">*</span>}</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Previous College Address {editable && <span className="text-red-500">*</span>}</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Previous College Start Date {editable && <span className="text-red-500">*</span>}</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Previous College End Date {editable && <span className="text-red-500">*</span>}</label>
