...
function TeacherModule() {
  ...
  // Derive options dynamically from existing teachers
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this faculty record?')) {
      setTeachers(teachers.filter((t) => t.empId !== id));
    }
  };
  ...
  const filteredTeachers = teachers.filter(teacher => {
    if (filters.empId && !teacher.empId.toLowerCase().includes(filters.empId.toLowerCase())) return false;
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredTeachers.length > 0 ? (
                filteredTeachers.map((teacher) => (
                  <tr key={teacher.empId} className='hover:bg-gray-50'>
                    <td className='px-6 py-4 text-sm font-medium text-gray-900 break-words'>
                      {teacher.empId}
                    </td>
                    <td className='px-6 py-4 text-sm text-gray-900 break-words'>
                      <div className='flex items-center gap-2'>
                        {teacher.image && <img src={teacher.image} alt='Faculty Avatar' className='w-8 h-8 rounded-full object-cover' />}
                        {teacher.name}
                      </div>
                    </td>
                  ...
                  <td colSpan="9" className="px-6 py-8 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-500">
                      ...
                      <h3 className='text-lg font-semibold mb-1'>
                        {teachers.length === 0 ? 'No faculty yet!' : 'No matching faculty found.'}
                      </h3>
                      <p className='mt-1 text-sm text-gray-600'>
                        {teachers.length === 0 ? 'Get started by adding a new faculty record' :
