...
const SUBJECT_OPTIONS = [
  ...
  'English Literature', 'History', 'Economics', 'Political Science',
  'Psychology', 'Sociology', 'Philosophy', 'Statistics', 'Accounting',
...
function AddTeacher({ onClose, onSave, existingTeachers }) {
  ...
  const validateForm = () => {
    const newErrors = {};
    const { empId, name, email, phone, password, subject, classes, classTeacher, image } = formData;
    ...
    if (existingTeachers.some(t => t.empId.toLowerCase() === trimmedEmpId.toLowerCase())) {
      newErrors.empId = 'Duplicate EMP ID found';
    }

    if (existingTeachers.some(t => t.email.toLowerCase() === trimmedEmail)) {
      newErrors.email = 'Duplicate Gmail ID found';
    }

    if (existingTeachers.some(t => t.phone === trimmedPhone)) {
