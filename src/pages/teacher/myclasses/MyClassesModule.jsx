...
const MyClassesModule = () => {
    // Mock current faculty ID.
    const currentFacultyId = "F125"; // Displaying courses for this faculty

    // Mock data for all courses in the system with simplified fields.
    const [allCourses, setAllCourses] = useState([
        {
            id: 'course_001',
            name: 'B.Sc Computer Science', // Program
            section: 'A', // Section
    ...
    const [formData, setFormData] = useState({
        ...
        section: '', // Section
        subject: '',
        facultyId: currentFacultyId, // Default to current faculty
    });

    // Filters courses to only show those assigned to the current faculty.
    const facultyCourses = useMemo(() => {
        return allCourses.filter(course => course.facultyId === currentFacultyId);
    }, [allCourses, currentFacultyId]);
    ...
    const handleCancelClick = () => {
        ...
        setFormData({ // Reset form data
            id: '', name: '', section: '', subject: '', facultyId: currentFacultyId
        });
    };
        ...
        <div className="px-0 sm:px-2 md:px-4 lg:p-6 flex flex-col gap-2 sm:gap-4 lg:gap-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-4 border-b-2 border-blue-500">
                <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 flex items-center gap-3">
                    <BookOpen size={36} className="text-blue-600" /> My Assigned Courses
                </h1>
            ...
            {showForm && (
                <div className="bg-white rounded-xl shadow-lg p-6 md:p-8 mb-8 border border-gray-200 animate-fade-in">
                    ...
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-2">Program (e.g., B.Sc Computer Science)</label>
                            <input
                                type="text"
                                name="name"
                                id="name"
