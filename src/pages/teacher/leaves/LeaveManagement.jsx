...
// Mock data for the current teacher (for display purposes)
...
// Mock data for teacher's own leave requests
...
const LeaveManagement = () => {
    ...
    const [newRequest, setNewRequest] = useState({
        ...
        reason: '', // Added for teacher's leave
        attachment: null, // For file upload simulation
    });

    // Simulate fetching teacher's leave requests on component mount
    useEffect(() => {
    ...
    const handleSubmitRequest = (e) => {
        ...
        const newId = teacherLeaveRequests.length > 0 ? Math.max(...teacherLeaveRequests.map(req => req.id)) + 1 : 1;

        const newTeacherRequest = {
            id: newId,
            applicantId: mockTeacherInfo.id, // Assign current teacher's ID
            applicantName: mockTeacherInfo.name, // Assign current teacher's name
        ...
        setTeacherLeaveRequests(prev => [newTeacherRequest, ...prev]); // Add new request to the beginning of the list
