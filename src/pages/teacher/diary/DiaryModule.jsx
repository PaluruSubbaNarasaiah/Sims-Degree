...
const DiaryModule = () => {
    ...
    const currentFacultyId = "faculty_F123";

    // --- State for Home Work Diary ---
    const [homeworkEntries, setHomeworkEntries] = useState([
        {
            id: 'hw1',
            facultyId: 'faculty_F123',
            date: '2025-06-27',
            classSelected: 'B.Sc Computer Science',
            sectionSelected: 'A',
            homeworkItems: [
                { subject: 'Computer Science', homework: 'Implement binary search tree with insertion and deletion operations.' },
                { subject: 'Mathematics', homework: 'Solve calculus problems on integration by parts (Chapter 8).' }
            ]
        },
        {
            id: 'hw2',
            facultyId: 'faculty_F123',
            date: '2025-06-27', // Same date as hw1
            classSelected: 'B.Com',
            sectionSelected: 'B',
            homeworkItems: [
                { subject: 'Economics', homework: 'Analyze market trends and prepare a report on supply-demand equilibrium.' }
            ]
        },
        {
            id: 'hw4', // Added a new entry for a different course but same date
            facultyId: 'faculty_F123',
            date: '2025-06-27',
            classSelected: 'B.Sc Physics',
            sectionSelected: 'A',
            homeworkItems: [
                { subject: 'Physics', homework: 'Solve quantum mechanics problems on wave functions and probability.' }
            ]
        },
        {
            id: 'hw3',
            facultyId: 'faculty_F123', // Changed to currentFacultyId for demonstration
            date: '2025-06-26',
            classSelected: 'B.Sc Statistics',
            sectionSelected: 'C',
            homeworkItems: [
                { subject: 'Statistics', homework: 'Complete regression analysis assignment using R programming.' }
            ]
        },
    ]);
    ...
    const [personalNotes, setPersonalNotes] = useState([
        { id: 'pn1', facultyId: 'faculty_F123', date: '2025-06-27', title: 'Faculty Meeting Notes', content: 'Discussed new assessment criteria. Follow up with department head regarding B.Sc Computer Science curriculum updates.' },
        { id: 'pn2', facultyId: 'faculty_F123', date: '2025-06-26', title: 'To-Do List', content: '1. Prepare lecture slides for Data Structures class. 2. Email students about project deadlines. 3. Grade programming assignments.' },
        { id: 'pn3', facultyId: 'faculty_F124', date: '2025-06-27', title: 'Research Ideas', content: 'Explore machine learning applications for final year projects.' },
    ]);
    ...
    const subjectOptions = [
        ...
        'Management', 'Accounting', 'Data Science', 'Software Engineering', 'English',
    ];
        ...
        assignmentsFromModal.forEach(assignment => {
            if (editingHomework && assignment.id === editingHomework.id) {
                updatedHomeworkEntries = updatedHomeworkEntries.map(entry =>
                    entry.id === editingHomework.id ? { ...entry, ...assignment, facultyId: currentFacultyId } : entry
                );
    ...
    const filteredHomeworkEntries = useMemo(() => {
        return homeworkEntries
            .filter(entry => entry.facultyId === currentFacultyId)
            .sort((a, b) => parseISO(b.date).getTime() - parseISO(a.date).getTime());
    }, [homeworkEntries, currentFacultyId]);
    ...
    const handleSavePersonalNote = (data) => {
        if (editingPersonalNote) {
            ...
            const newNote = { ...data, id: `pn${Date.now()}`, facultyId: currentFacultyId };
    ...
    const filteredPersonalNotes = useMemo(() => {
        return personalNotes
            .filter(note => note.facultyId === currentFacultyId)
            .sort((a, b) => parseISO(b.date).getTime() - parseISO(a.date).getTime());
    }, [personalNotes, currentFacultyId]);
                                                ...
                                                <div className="flex justify-between items-start mb-2">
                                                    <div className="flex-1 min-w-0">
                                                        <h4 className="text-base font-semibold text-gray-700">
                                                            Program: {entry.classSelected}, Section: {entry.sectionSelected}
                                                        </h4>
                                                    </div>
